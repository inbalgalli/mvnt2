...
public class FiboHeapTest {
    ...
    public static int getNumMarked(FibonacciHeap heap) {
        return (heap.potential() - heap.getNumOfTrees()) / 2;
    }
...
//                FiboHeapPrinter.printHeap(heap);
    ...
    public static void testMeld(FibonacciHeap heap, int upperBound) {
        ...
        int size = heap.size(), numOfTrees = heap.getNumOfTrees(), numMarked = getNumMarked(heap);
        heap.meld(heap2);
        if (heap.size() != size + heap2.size()) {
            throw new RuntimeException("size after meld is not sum of both heaps");
        }
        if (heap.getNumOfTrees() != numOfTrees + heap2.getNumOfTrees()) {
    ...
    public static void testKMin(FibonacciHeap heap) {
        if (heap.size() == 0) {
            ...
            FibonacciHeap heaps[] = new FibonacciHeap[heap.getNumOfTrees()];
            int heapIndex = 0;
            do {
                next = cur.getNext();
                cur.setPrev(cur);
    ...
    public static void testNumOfTrees(FibonacciHeap heap) {
        ...
        if (actualNumOfTrees != heap.getNumOfTrees()) {
            throw new RuntimeException(String.format("should have %s trees but counted %s", heap.getNumOfTrees(), actualNumOfTrees));
        }
    }
    ...
    public static void testHeapFields(FibonacciHeap heap, ExpectedFields expected) {
        if (expected.numOfTrees != null && expected.numOfTrees != heap.getNumOfTrees()) {
            throw new RuntimeException(
                    String.format("wrong amount of trees. found %d but expected %d",
                            heap.getNumOfTrees(), expected.numOfTrees)
            );
        }
